...
namespace Apps.SFTP.Invocables
{
    public class SFTPInvocable : BaseInvocable
    {
        ...
        protected T UseClient<T>(Func<BlackbirdSftpClient, T> action)
        {
            ...
            {
                throw new PluginMisconfigurationException($"The server denied access to the current connection credentials. Please update the connection.");
            }
            catch (SftpPathNotFoundException ex)
            {
                throw new PluginMisconfigurationException($"The provided file or path does not exist on the server. Please make sure the path is valid.");
            }
